...
import { collection, query, orderBy, onSnapshot, where, getDocs, doc, updateDoc, deleteDoc, serverTimestamp } from 'firebase/firestore'
...
function AdminRecords() {
  ...
  const [editForm, setEditForm] = useState({ notes: '', serviceType: '' })
  ...
  useEffect(() => {
    const q = query(collection(db, 'patients'), orderBy('firstname'))
  ...
  const cancelEditing = () => {
    ...
    setEditForm({ notes: '', serviceType: '' })
  }

  const handleSaveEdit = async (visitId) => {
    setIsSaving(true)
    try {
      await updateDoc(doc(db, 'queues', visitId), {
  ...
  const handleDeleteRecord = async (visitId) => {
    if (!window.confirm('คุณแน่ใจหรือไม่ที่จะลบประวัติการรักษานี้?\n\n(การลบจะไม่สามารถกู้คืนได้ และอาจส่งผลต่อข้อมูลทางสถิติ)')) {
      return
    }
    try {
      await deleteDoc(doc(db, 'queues', visitId))
      alert('ลบประวัติการรักษาสำเร็จ')
    } catch (err) {
        ...
        <div className="w-full md:w-2/3 flex flex-col h-[85vh]">
          {!selectedPatient ? (
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col h-full overflow-hidden animate-[fadeIn_0.2s_ease]">
              ...
              {/* Header Info */}
              <div className="p-6 bg-gradient-to-r from-slate-700 to-slate-900 text-white flex-shrink-0">
                 <div className="flex justify-between items-start">
                   <div>
                     ...
                     <div className="mt-4 flex gap-6 text-sm text-slate-100">
                    ...
                    <div className="mb-8">
                       ...
                       {emrFiles.length === 0 ? (
                         ...
                         <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                           {emrFiles.map(f => (
                             <a key={f.id} href={f.url} target="_blank" rel="noreferrer" className="flex flex-col bg-white border border-gray-200 p-3 rounded-xl hover:border-slate-400 hover:shadow-md transition">
                                ...
                                <div className="text-xs font-bold text-slate-700 truncate mb-1">{f.type}</div>
                                   ...
                                   {isEditing ? (
                                     <div className="animate-[fadeIn_0.2s_ease]">
                                        <div className="flex justify-between items-center mb-3 border-b pb-2">
                                          <h4 className="font-bold text-slate-800">แก้ไขข้อมูลการรักษา</h4>
                                          <span className="text-xs text-gray-400">{vDate}</span>
                                        </div>
                                        
                                        <div className="mb-3">
                                          <label className="block text-xs font-bold text-gray-600 mb-1">บริการ (Service Type)</label>
                                          <input 
                                            type="text" 
                                        ...
                                        <div className="mb-4">
                                          <label className="block text-xs font-bold text-gray-600 mb-1">บันทึกแพทย์ (Notes)</label>
                                          <textarea 
                                        ...
                                        <div className="flex justify-end gap-2">
                                          <button onClick={cancelEditing} className="px-3 py-1.5 text-xs font-bold bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-lg">ยกเลิก</button>
                                          <button 
                                     ...
                                     <>
                                       <div className="flex items-start justify-between mb-1">
                                         <div>
                                           <div className="font-bold text-slate-800">{vDate}</div>
                                         </div>
